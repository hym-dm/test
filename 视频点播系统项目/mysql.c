...
int main()
{
    ...
    //连接服务器 mysql_real_connect(句柄，服务器IP，用户名，密码，库名称，端口，套接字文件，客户端标志)
    if(mysql_real_connect(mysql,"127.0.0.1","root","hym123...","vod_system",0,NULL,0)==NULL)
    {
    ...
    //读取结果集条数mysql_num_rows(结果集)
    //读取结果集列数mysql_num_fields(结果集)
